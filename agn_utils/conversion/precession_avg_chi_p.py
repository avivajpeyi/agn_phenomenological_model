...
def ftor_PN(f, M_msun, q, chi1, chi2, theta1, theta2, deltaphi):
    ...
    r = (mom) ** (-2. / 3.) * (1. \
                               - (1. / 3.) * (3. - eta) * mom ** (2. / 3.) \
                               - (1. / 3.) * (chi1 * ct1 * (
                    2. * m1 ** 2. + 3. * eta) + chi2 * ct2 * (
                                                      2. * m2 ** 2. + 3. * eta)) * mom \
                               + (eta * (
                    19. / 4. + eta / 9.) - eta * chi1 * chi2 / 2. * (
                                          ct12 - 3. * ct1 * ct2)) * mom ** (4. / 3.) \
                               )
    return r
...
def chip_asymptotic(q, chi1, chi2, theta1, theta2):
    ...
    term1, term2 = chip_terms(q, chi1, chi2, theta1, theta2)
    return (np.abs(term1 - term2) * scipy.special.ellipe(
        -4 * term1 * term2 / (term1 - term2) ** 2) + np.abs(
        term1 + term2) * scipy.special.ellipe(
        4 * term1 * term2 / (term1 + term2) ** 2)) / np.pi


def chip_thomas(q, chi1, chi2, theta1, theta2, deltaphi):
    factor_term = 1 / (1 + q) ** 2
    term1 = (chi1 * np.sin(theta1)) ** 2
    term2 = (q * chi2 * np.sin(theta2)) ** 2
    term3 = 2 * q * chi1 * chi2 * np.sin(theta1) * np.sin(theta2) * np.cos(deltaphi)
    return factor_term * np.sqrt(term1 + term2 + term3)
...
def chip_averaged(q, chi1, chi2, theta1, theta2, deltaphi, r=None, fref=None,
    ...
    if r is None and fref is None:
        raise ValueError
    elif r is not None and fref is not None:
        raise ValueError
    if r is None:
    ...
    # Eq A2
    J = (L ** 2 + S1 ** 2 + S2 ** 2 + 2 * L * (
            S1 * np.cos(theta1) + S2 * np.cos(theta2)) + 2 * S1 * S2 * (
                 np.sin(theta1) * np.sin(theta2) * np.cos(deltaphi) + np.cos(
             theta1) * np.cos(theta2))) ** 0.5
    ...
    def integrand_numerator(S):
        '''chip(S)/dSdt(S)'''
        # Eq A3
        theta1ofS = np.arccos((1 / (2 * (1 - q) * S1)) * (
                (J ** 2 - L ** 2 - S ** 2) / L - 2 * q * chieff / (1 + q)))
        # Eq A4
        theta2ofS = np.arccos((q / (2 * (1 - q) * S2)) * (
                -(J ** 2 - L ** 2 - S ** 2) / L + 2 * chieff / (1 + q)))
        # Eq A5
        deltaphiofS = np.arccos((S ** 2 - S1 ** 2 - S2 ** 2 - 2 * S1 * S2 * np.cos(
            theta1ofS) * np.cos(theta2ofS)) / (2 * S1 * S2 * np.sin(theta1ofS) * np.sin(
            theta2ofS)))
...
if __name__ == "__main__":
    ...
    if True:
        ...
        print("Generalized chip:",
              chip_generalized(q, chi1, chi2, theta1, theta2, deltaphi))
        print("Averaged chip:",
              chip_averaged(q, chi1, chi2, theta1, theta2, deltaphi, fref=fref,
                            M_msun=M_msun))
