...
def get_bbh_population_from_agn_prior(num_samples=10000, population_params={}):
    ...
    s1 = scale_vector(a_1, np.array(make_spin_vector(tilt_1, phi_1)).transpose())
    s2 = scale_vector(a_2, np.array(make_spin_vector(tilt_2, phi_2)).transpose())
    s1x, s1y, s1z = s1[:, 0], s1[:, 1], s1[:, 2]
    s2x, s2y, s2z = s2[:, 0], s2[:, 1], s2[:, 2]
    theta_jn, phi_jl, _, _, _, _, _ = transform_component_spins(
        incl=incl, S1x=s1x, S1y=s1y, S1z=s1z,
    ...
    s[LUMINOSITY_DISTANCE] = bilby.gw.conversion.redshift_to_luminosity_distance(z)
    s[TILT_1] = tilt_1
    s[TILT_2] = tilt_2
