...
def get_bbh_population_from_prior(
    num_samples: int, prior_fname: str
) -> pd.DataFrame:
...
def get_bbh_population_from_agn_params(
    num_samples: int, sigma_1: float, sigma_12: float
) -> pd.DataFrame:
    """
    ...
    samples = get_bbh_population_from_agn_prior(
        num_samples=num_samples, population_params=pop_params
    )
    return samples
