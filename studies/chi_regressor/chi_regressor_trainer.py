...
from agn_utils.chi_regressor import AvailibleRegressors, chi_regressor_trainer, \
    load_model, prediction_for_different_sigma
...
def create_parser_and_read_args():
    ...
    parser.add_argument("--outdir", help="model's outdir'", type=str,
                        default='my.model')
...
def main():
    ...
    if args.make_dag:
        ...
        chi_regressor_trainer(
            outdir=args.outdir, model_type=model_type,
            training_fname=TRAINING_DATA, n_samples=args.nsamples)
        regressor = load_model(outdir=args.outdir, model_type=model_type)
        for s in [[0.1, 0.8], [3, 0.3], [1, 1]]:
            prediction_for_different_sigma(
                sigma_1=s[0], sigma_12=s[1], n=5000, regressor=regressor, save=True
            )
